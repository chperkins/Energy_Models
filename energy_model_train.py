...
Original file is located at
    https://colab.research.google.com/drive/1vj6WuSih6UBoom0SwxiIiPIhM7viJLUX
"""
...
config = {
  ...
  "z_size": 50, #number of latent variables in latent z for coopnets
  'img_size': 32, #width/lenght of image
  ...
  'init': 'uniform', #initialize samples - uniform, gaussian, generator, persistant
  ...
  'sgd_beta': 0, #momentum for sgd of synthesized szamples
  'test_size': 25000, #test size output of finished models for FID scores
  'data': 'mnist', #dataset - usually mnist or cifar10
...
EXP_DIR = ebm_dir+'out_data/'+str(config['data'])+'_'+config['init']+'_'+str(config['langevin_step_size_des'])+'_step_'+str(config['data_epsilon'])+'_noise_'+str(config['lr_des'])+'_lr_'+str(config['langevin_step_num_des'])+'_'+str(config['minimizer'])+'_beta_'+str(config['sgd_beta'])+'_'+str(config['spec_norm'])+'_energy_'+str(config['l2_energy_reg'])+'_'+config['with_noise']+'/'

def saveSampleResults(sample_results, filename, col_num=1, margin_syn=2):
...
class Descriptor(nn.Module):
  def __init__(self):
    ...
    self.conv1 = nn.Conv2d(config['num_channels'], 64, kernel_size=5, stride=2, padding=1)
...
class Generator(nn.Module):
  def __init__(self):
    ...
    self.convt1 = nn.ConvTranspose2d(config['z_size'], 512, kernel_size=4, stride=1, padding=0)
    ...
    self.convt4 = nn.ConvTranspose2d(128, config['num_channels'], kernel_size=5, stride=2, padding=2, output_padding=1)
...
def save_data_plot(data_in, dir_in, name_in):
  ...
  ax.plot(data_in[200:])
  fig.savefig(dir_in+'post_200_'+name_in)   # save the figure to file
  plt.close(fig)    # close the figure window

  fig, ax = plt.subplots( nrows=1, ncols=1 )  # create figure & 1 axis
  ax.plot(data_in)
  fig.savefig(dir_in+'all_'+name_in)   # save the figure to file
  plt.close(fig)    # close the figure window
...
class CoopNets(nn.Module):
  ...
  def langevin_dynamics_descriptor(self, x):
    ...
    for i in range(config['langevin_step_num_des']):
      ...
      #alternatively run SGD with/without momementum/n oise
  ...
  def train(self):
    ...
    self.descriptor = Descriptor().cuda() #initialize nets
    self.generator = Generator().cuda()
    ...
    for i in range(config['num_iter']):
      ...
      random_noise = t.randn(100, config['num_channels'], config['img_size'], config['img_size'])
      ...
      des_loss = (revised_feature.mean() - obs_feature.mean()).sum() + config['l2_energy_reg']*(revised_feature.norm().sum() + obs_feature.norm().sum())
      ...
      if (i + 1) == 1 or (i+1) % 1000 == 0 or i == 199 or i == 99:
    ...
    save_data_plot(des_loss_epoch, EXP_DIR, 'des_loss.png')
    save_data_plot(gen_loss_epoch, EXP_DIR, 'gen_loss.png')
    save_data_plot(recon_loss_epoch, EXP_DIR, 'recon_loss.png')

    save_data_plot(energy_data, EXP_DIR, 'energy_data.png')
    save_data_plot(energy_start, EXP_DIR, 'energy_start.png')
    save_data_plot(energy_end, EXP_DIR, 'energy_end.png')
    save_data_plot(gradient_start, EXP_DIR, 'gradient_start.png')
    save_data_plot(gradient_end, EXP_DIR, 'gradient_end.png')

  #output test images from completed model
  def test(self, d_path, g_path, test_size, init_type):
        
    if init_type == 'generator':
      generator = t.load(g_path).eval()
